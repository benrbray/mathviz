...
interface NormalVectorProps {
  dataKey: string;
  value: THREE.Vector3;
  onValueChange: (value: THREE.Vector3) => void;
  spec: NormalVectorSpec;
  visible: boolean;
...
/*
 * TODO (Ben @ 2025/05/27)
 * This implementation suffers from "Gimbal Lock".
 * Can we re-use TrackballControls to avoid this problem?
 */

export const NormalVectorControl: React.FC<NormalVectorProps> = (props) => {
  ...
  const hAngleRef = R.useRef(NaN);
  const vAngleRef = R.useRef(NaN);
  ...
  return (
    <label className={"normalLabel" /*s("normalLabel")*/}>
      {/* {svgLabel || firstUpper(spec.label ?? dataKey)} */}
      <div className={"normal" /*s("normal")*/} onMouseDown={onMouseDown} onTouchStart={onMouseDown}>
