...
import { getBasicMaterial, type IVector3, parseVector } from "../utils";
...
export const MathLabel: React.FC<Props> = (props) => {
  ...
  const geometries = R.useMemo(() => {
    let svg = getMathSvg(props.label);
    if (!svg) return [];
    svg = svg.replaceAll('currentColor', 'red');
  ...
  const groupRef = R.useRef<THREE.Group>(null);
